...
4. Start the MCP server with `--runtime-data /absolute/path/snapshot.json`, or set `WOW_API_RUNTIME_DATA` to the file path. Also pass `--channel forever` for Forever observations. Restart the server when replacing the file.
